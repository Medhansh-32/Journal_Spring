...
public class JournalEntryControllerV2 {
    ...
    @GetMapping
    public String getAllJournalEntriesofUser(Model model) {
        org.springframework.security.core.Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String userName = authentication.getName();
        User user = userService.findByUserName(userName);
        List<JournalEntry> allData = user.getJournalEntries();
        model.addAttribute("entries", allData);
        return "journal"; // Return the journal.html template
    }
    ...
    @GetMapping("/id/{myId}")
    public String getJournalEntry(@PathVariable ObjectId myId, Model model) {
        org.springframework.security.core.Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String userName = authentication.getName();
        User user = userService.findByUserName(userName);
        ...
        if (!collect.isEmpty()) {
            ...
            if (journalEntry.isPresent()) {
                ...
                return "journalEntry"; // Return the journalEntry.html template
            }
        }
        return "error"; // Return an error template
    }
